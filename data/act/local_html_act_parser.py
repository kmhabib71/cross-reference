...
class LocalActParser:
    ...
    def parse_section_stream(self, section_div: Tag, section_title: str) -> Tuple[str, List[LegalTable], List[Subsection], List[Clause], List[Article]]:
        ...
        for child in txt_details.children:
            ...
            if child.name in ['p', 'div', 'li']:
                # 1) Attach any nested tables inside this block first
                try:
                ...
                # 2) Build text without table HTML to avoid duplication
                try:
                ...
                # Detect markers
                # Normalize by removing any leading section number like "১। " so that leading (১) is detectable
    ...
    def extract_section_from_div(self, section_div: Tag) -> Optional[Section]:
        ...
        # Heuristic: try first paragraph to catch leading "১।" or "1."
        try:
        ...
        # Final fallback for classic Section 1 phrasing
        if not section_number and re.search(r'এই আইন.*অভিহিত হইবে', content_text):
            section_number = '১'
